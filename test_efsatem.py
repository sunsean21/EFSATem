...
parser.add_argument("--data_path", type=str, default="data/NTURain/image/", help='path to training data')
parser.add_argument("--event_path", type=str, default="data/NTURain/event/", help='path to event data')
parser.add_argument("--save_path", type=str, default="output/mpevNet/out_s7_i123_100_sa_resume_e", help='path to save results')
...
parser.add_argument("--dim_feature", type=int, default=32, help='dimension of hidden features')
...
#    os.makedirs(opt.save_path, exist_ok=True)
    ...
    # Build model
    print('Loading model ...\n')
    model = Model(dim=opt.dim_feature)
    print_network(model)
    if opt.use_GPU:
        model = model.cuda()
    if opt.model_epoch == 0:
        state_dict = torch.load(Path(opt.logdir) / 'net_latest.pth')
    else:
        state_dict = torch.load(Path(opt.logdir) / 'net_epoch{}.pth'.format(opt.model_epoch))
        ...
        for jj, img_path in enumerate(files):
            ...
            if ( (jj + 1) % opt.seq_len == 0 ) or jj + 1 == len(files): 
                ...
                with torch.no_grad(): #
                    ...
                    '''
                    latent_prev = Variable(torch.zeros(1, int(opt.dim_feature*2**3), h//2**3, w//2**3)).to(seq_img) if jj + 1 == opt.seq_len else latent_prev
                    out, latent_prev = model(seq_img, seq_event[:,[0,2]], latent_prev) # 1 x c x s x h x w
