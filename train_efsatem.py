...
parser = argparse.ArgumentParser(description="EFSATeM_train")
...
parser.add_argument("--save_path", type=str, default="logs/s7_i6_100_ev_sa2", help='path to save models and log files')
parser.add_argument("--save_freq",type=int,default=1,help='save intermediate model')
parser.add_argument("--data_path",type=str, default="/home1/ssq/proj1/evnet/data/",help='path to training data')
...
if opt.use_gpu:
    ...
    device_ids = [int(i) for i in opt.gpu_id.strip().split(',')]
...
def main():
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode="max", factor=0.1, patience=1, threshold=1e-6)  # learning rates
    # record training
    ...
    if initial_epoch > 0:
        ...
        state_dict = torch.load(os.path.join(opt.save_path, 'net_epoch%d.pth' % initial_epoch))
